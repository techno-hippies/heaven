...
const KIDS_TIMELINE_OPTIONS = [
  { value: 1, label: 'Want kids' },
  { value: 2, label: 'Want kids someday' },
  { value: 3, label: 'Open to kids' },
  { value: 4, label: "Don't want kids" },
  { value: 5, label: 'Not sure yet' },
...
interface KidsTimelinePageProps {
  step?: number
  totalSteps?: number
  onContinue?: (timeline: number, visibility: Visibility) => void
}

export const KidsTimelinePage: Component<KidsTimelinePageProps> = (props) => {
  const [timeline, setTimeline] = createSignal<number | null>(null)
  const [visibility, setVisibility] = createSignal<Visibility>('shared')

  const canContinue = () => timeline() !== null

  const handleContinue = () => {
    if (timeline() !== null) {
      props.onContinue?.(timeline()!, visibility())
    }
  }
        ...
        <div class="w-full max-w-lg mx-auto px-6 py-8">
          ...
          <div class="mb-10">
            <div class="flex flex-wrap gap-3">
              <For each={KIDS_TIMELINE_OPTIONS}>
                {(option) => {
...
export default KidsTimelinePage
