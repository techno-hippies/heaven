      ...
      >
        ...
        <div class="flex-1 min-h-0 overflow-y-auto lg:flex-initial lg:overflow-visible lg:max-w-md">
          ...
          <div class="px-6 py-6 lg:px-0 lg:pb-0 lg:mt-2">
            ...
            {/* 2-col field grid */}
            <div class={cn('grid grid-cols-2 gap-x-8 gap-y-4', profile().bio ? 'mt-4' : '')}>
              <Show when={profile().ageBucket}>
                <ProfileBadge category="Age" value={AGE_BUCKET_LABELS[profile().ageBucket!]} attested stacked />
              </Show>
              <Show when={profile().biologicalSex !== undefined}>
                <ProfileBadge category="Sex" value={BIOLOGICAL_SEX_LABELS[profile().biologicalSex!]} attested stacked />
              </Show>
              <Show when={profile().nationality}>
                <ProfileBadge category="Nationality" value={NATIONALITY_LABELS[profile().nationality!] || profile().nationality!} attested stacked />
              </Show>
              <Show when={profile().genderIdentity}>
                <ProfileBadge category="Gender" value={GENDER_IDENTITY_LABELS[profile().genderIdentity!]} stacked />
              </Show>
              <Show when={profile().lookingFor}>
                <ProfileBadge category="Seeking" value={LOOKING_FOR_LABELS[profile().lookingFor!]} stacked />
              </Show>
              <Show when={profile().bodyBucket}>
                <ProfileBadge category="Body" value={BODY_BUCKET_LABELS[profile().bodyBucket!]} stacked />
              </Show>
              <Show when={profile().fitnessBucket}>
                <ProfileBadge category="Fitness" value={FITNESS_BUCKET_LABELS[profile().fitnessBucket!]} stacked />
              </Show>
              <Show when={profile().smoking}>
                <ProfileBadge category="Smoking" value={SMOKING_LABELS[profile().smoking!]} stacked />
              </Show>
              <Show when={profile().drinking}>
                <ProfileBadge category="Drinking" value={DRINKING_LABELS[profile().drinking!]} stacked />
              </Show>
              <Show when={profile().kids}>
                <ProfileBadge category="Kids" value={KIDS_LABELS[profile().kids!]} stacked />
              </Show>
              <Show when={profile().kidsTimeline}>
                <ProfileBadge category="Kids timeline" value={KIDS_TIMELINE_LABELS[profile().kidsTimeline!]} stacked />
              </Show>
              <Show when={profile().relationshipStructure}>
                <ProfileBadge category="Relationship" value={RELATIONSHIP_STRUCTURE_LABELS[profile().relationshipStructure!]} stacked />
              </Show>
              <Show when={profile().religion}>
                <ProfileBadge category="Religion" value={RELIGION_LABELS[profile().religion!]} stacked />
              </Show>
              <Show when={profile().kinkLevel}>
                <ProfileBadge category="Kink" value={KINK_LEVEL_LABELS[profile().kinkLevel!]} stacked />
              </Show>
              <Show when={profile().incomeRange}>
                <ProfileBadge category="Income" value={INCOME_RANGE_LABELS[profile().incomeRange!]} stacked />
              </Show>
            </div>
