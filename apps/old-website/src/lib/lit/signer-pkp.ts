...
/**
 * Sign a digest (EIP-712 hash) using PKP
 * Returns a 65-byte ECDSA signature (r+s+v)
 */
export async function signDigestWithPKP(
  pkpInfo: PKPInfo,
  authData: AuthData,
  digest: Hex
): Promise<Hex> {
  if (IS_DEV) console.log('[Lit] Signing digest with PKP...', digest)
  ...
  try {
    ...
    const result = await litClient.executeJs({
      ...
      authContext: authContext,
      jsParams: {
        toSign: Array.from(hexToBytes(digest)),
        publicKey: pkpInfo.publicKey,
      },
    })

    if (IS_DEV) console.log('[Lit] Digest sign result:', result)

    if (result.signatures && result.signatures.digestSig) {
      const sig = result.signatures.digestSig
      ...
      if (sig.signature && recoveryId !== undefined) {
        ...
        const signature = `0x${sigHex}${v}` as Hex

        if (IS_DEV) console.log('[Lit] Digest signed successfully')
        return signature
      }
      ...
      if (sig.r && sig.s && recoveryId !== undefined) {
        ...
        if (IS_DEV) console.log('[Lit] Digest signed successfully (r,s format)')
        return signature
      }
    }
    ...
    console.error('[Lit] Failed to sign digest:', error)
    throw new Error(
      `Failed to sign digest: ${error instanceof Error ? error.message : 'Unknown error'}`
    )
  }
}
