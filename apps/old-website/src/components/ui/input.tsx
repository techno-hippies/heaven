...
export const Spinner: Component<SpinnerProps> = (props) => {
  const sizeClass = () => {
    switch (props.size ?? 'sm') {
      case 'xs': return 'w-3 h-3 border-[1.5px]'
      case 'sm': return 'w-4 h-4 border-2'
      case 'md': return 'w-5 h-5 border-2'
    }
  }
    ...
    <div class={cn('h-6 flex items-center gap-2', props.class)}>
      ...
      {props.state === 'valid' && (
        <>
          ...
          <span class="text-sm text-green-600">{validMsg()}</span>
        </>
      )}
      {props.state === 'invalid' && (
        <>
          ...
          <span class="text-sm text-destructive">{invalidMsg()}</span>
        </>
      )}
    </div>
  )
