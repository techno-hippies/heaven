...
export interface IconButtonProps extends Omit<JSX.ButtonHTMLAttributes<HTMLButtonElement>, 'children'> {
  icon: IconName
  size?: 'sm' | 'md' | 'lg'
...
export const IconButton: Component<IconButtonProps> = (props) => {
  const [local, others] = splitProps(props, ['icon', 'size', 'variant', 'class'])
  const size = () => iconButtonSizes[local.size ?? 'sm']
  ...
  return (
    <Button
      ...
      {...others}
    >
      <Icon name={local.icon} class="text-xl" />
    </Button>
  )
}
