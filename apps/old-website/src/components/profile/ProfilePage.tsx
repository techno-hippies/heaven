...
export interface ProfilePageProps {
  ...
  /** Survey completion status (only shown for own profile) */
      ...
      <div class="min-h-screen flex flex-col lg:flex-row lg:items-center lg:justify-center lg:p-12 lg:gap-12">
        ...
        <div class="flex-1 lg:max-w-md">
          ...
          <div class="px-6 py-6 pb-28 lg:px-0 lg:pb-0 lg:mt-2">
            ...
            {/* 2-col field grid */}
            <div class={cn('grid grid-cols-2 gap-x-8 gap-y-4', props.profile.bio ? 'mt-4' : '')}>
              <Show when={props.profile.regionBucket && props.profile.regionBucket > 0}>
                <ProfileBadge category="Region" value={REGION_BUCKET_LABELS[props.profile.regionBucket as RegionBucket]} stacked />
              </Show>
              <Show when={props.profile.ageBucket && props.profile.ageBucket > 0}>
                <ProfileBadge category="Age" value={AGE_BUCKET_LABELS[props.profile.ageBucket]} attested stacked />
              </Show>
              <Show when={props.profile.genderIdentity && props.profile.genderIdentity > 0}>
                <ProfileBadge category="Gender" value={GENDER_IDENTITY_LABELS[props.profile.genderIdentity]} stacked />
              </Show>
              <Show when={props.profile.lookingFor && props.profile.lookingFor > 0}>
                <ProfileBadge category="Looking for" value={LOOKING_FOR_LABELS[props.profile.lookingFor]} stacked />
              </Show>
            </div>

            {/* Survey Status - only shown for own profile */}
            <Show when={props.isOwnProfile && props.surveyStatus !== undefined}>
              <div class="mt-6 p-4 rounded-2xl border border-border bg-card/50">
                <Show
                  when={props.surveyStatus?.completed}
                  fallback={
                    <div class="flex items-center justify-between">
                      <div>
                        <p class="text-sm font-medium text-foreground">Personality Survey</p>
                        <p class="text-xs text-muted-foreground mt-0.5">
                          Complete your survey to improve matches
                        </p>
                      </div>
                      <Button
                        variant="default"
                        size="sm"
                        onClick={props.onTakeSurvey}
                      >
                        Take Survey
                      </Button>
                    </div>
                ...
                >
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      <p class="text-sm font-medium text-foreground">Survey Completed</p>
                      <p class="text-xs text-muted-foreground">
                        Your encrypted preferences are stored on-chain
                      </p>
                    </div>
                  </div>
                </Show>
