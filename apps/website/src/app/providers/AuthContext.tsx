...
import { clearSession, isSessionExpiringSoon, loadSession, saveSession } from '@/lib/lit/storage'
import {
  wagmiConfig,
  getWalletClient,
  watchAccount,
...
export interface AuthContextType {
  ...
  getSigningAuthData: (options?: { force?: boolean }) => Promise<AuthData>
  openAuthDialog: () => void
...
export const AuthProvider: ParentComponent = (props) => {
  ...
  onMount(async () => {
    ...
    // Rebuild PKP auth context for decrypt operations (lazy load Lit SDK)
    try {
      const lit = await loadLit()
      await lit.createPKPAuthContext(session.pkpInfo, session.authData)
      if (import.meta.env.DEV) {
        console.log('[Auth] Restored PKP auth context from session')
      }
    } catch (err) {
      // Auth context rebuild failed - user may need to re-authenticate for decrypt
  ...
  // Get auth data for PKP signing (refresh when needed)
  async function getSigningAuthData(options?: { force?: boolean }): Promise<AuthData> {
    const currentPkpInfo = pkpInfo()
    const existingAuthData = authData()
    if (!currentPkpInfo || !existingAuthData) {
      throw new Error('Not authenticated')
    }

    const shouldRefresh = options?.force || isSessionExpiringSoon()
    if (existingAuthData.authMethodType !== 3 || !shouldRefresh) {
  ...
  async function signMessage(message: string): Promise<string> {
    ...
    const lit = await loadLit()
    const signingAuthData = await getSigningAuthData()

    return lit.signMessageWithPKP(currentPkpInfo, signingAuthData, message)
  }
  ...
  if (!context) {
    ...
    if (import.meta.hot) {
      ...
      return {
        ...
        getSigningAuthData: async () => { throw new Error('Auth context not ready') },
        openAuthDialog: () => {},
        closeAuthDialog: () => {},
