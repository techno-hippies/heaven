...
type AuthAction = 'register' | 'signIn' | 'wallet' | null
...
interface AuthModalProps {
  ...
  /** Optional initial state for previews (e.g. Storybook). */
...
export const AuthModal: Component<AuthModalProps> = (props) => {
  ...
  const [action, setAction] = createSignal<AuthAction>(null)
  ...
  createEffect(() => {
    if (props.open) {
      setState('idle')
      setAction(props.initialAction ?? null)
      setMethod(props.initialMethod ?? null)
    }
  })
  ...
  function getDescription() {
    ...
    if (state() === 'complete') return `You're now connected to ${APP_NAME}.`
    ...
    return `Choose how you want to connect to ${APP_NAME}.`
  }
